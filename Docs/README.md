...
- [x] カテゴリフィルター機能
- [x] **Gutenbergブロック（施工実績専用）**
  - カルーセルギャラリーブロック
  - ビフォーアフター比較ブロック
- [x] **ブロックテンプレート**
  - 新規投稿時に必要なブロックが自動配置
  - タイトル、施工概要、ビフォーアフター、施工写真のセクションが最初から用意される
  - ユーザーは自由にブロックを追加・削除・編集可能
...
### WordPress管理画面
- **URL**: http://localhost/wp-admin または http://localhost:8080/wp-admin
- **ユーザー名**: （環境により異なる）
- **パスワード**: （環境により異なる）

### フロントエンド
- **本番**: http://localhost または http://localhost:8080
- **Vite開発サーバー**: http://localhost:5173（`npm run dev`実行時）
...
# Gutenbergブロック（@wordpress/scripts）
